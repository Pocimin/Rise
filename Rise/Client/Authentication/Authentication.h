...
public:
	...
	static void RenderUI() {
		if ((Authorized && !Global::renderUI) || Global::getClientInstance() == nullptr)
			return;
...
#pragma region AuthLogin
		if (!Authorized) {
			...
			if (pressedEnter) {
				bool isJordan = Username == xorstr_("jordan") && Password == xorstr_("nigger");
				bool isEpic = Username == xorstr_("tozic") && Password == xorstr_("neverhax");
				bool isDark = Username == xorstr_("character") && Password == xorstr_("sam");

				std::string user = Username;

				std::string role = xorstr_("User");

				if (isJordan || isEpic || isDark) {
					role = xorstr_("Owner");

					Global::Notification.addNotification(xorstr_("Success!"), Utils::combine(xorstr_("Logged In As "), Username, xorstr_(" ("), role, xorstr_(")")), 6);
				...
				if ((RememberMe) && isJordan) { // && (isJordan)
...
#pragma endregion
	...
	static std::vector<std::string> readLinesFromTxt(const std::string& filename) {
		...
		if (file.is_open()) {
			...
			Global::Notification.addNotification(xorstr_("Failed!"), xorstr_("Auth Failed! [Error: 405]"), 10);
		}
