...
public:
	static void renderNotifications() {
		...
		float yOffset = Global::getClientInstance()->getGuiData()->getmcResolution().y - 40;
		...
		for (auto& notif : notifications) {
			...
			const float firstPos = Global::getClientInstance()->getGuiData()->getmcResolution().x - thelen - 8.f;
			const float lastPos = Global::getClientInstance()->getGuiData()->getmcResolution().x + 8.f;
			...
			Vector4<float> rect2(rect.x, rect.y, (posX + thelen * (elapsedTime / duration)), rect.w);
