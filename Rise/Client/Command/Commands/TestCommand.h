...
public:
    ...
    bool execute(std::vector<std::string> cmd) override
    {
        ...
        //auto PlayerHealth = Global::getClientInstance()->getLocalPlayer()->getAttribute(AttributeID::Health).mCurrentValue;
        //ChatUtils::sendMessage(Utils::combine("Test: ", std::to_string(PlayerHealth).c_str()));


        //Block* block = Global::getClientInstance()->getBlockSource()->getBlock(Global::getClientInstance()->getLocalPlayer()->getLevel()->getHitResult()->IBlockPos);
