...
public:
    ...
    {
        addEnum("Mode", "The type of fly", { "Vanilla", "Flareon", "silly", "funny", "lol" }, &mode);
...
public:
    ...
    void onEnabled() override {
        ...
        StartPosition = Global::getClientInstance()->getLocalPlayer()->getRenderPosition().y;

        oldPosition = Global::getClientInstance()->getLocalPlayer()->getRenderPosition();

        desyncPosition = Global::getClientInstance()->getLocalPlayer()->getRenderPosition();
        desyncRotation.x = Global::getClientInstance()->getLocalPlayer()->getComponent<ActorRotationComponent>()->rotation.x; // Pitch
        desyncRotation.y = Global::getClientInstance()->getLocalPlayer()->getComponent<MobBodyRotationComponent>()->bodyRot; // Body
        desyncRotation.z = Global::getClientInstance()->getLocalPlayer()->getComponent<ActorHeadRotationComponent>()->rotation.x; // Head
        ...
        if (mode == 1) {
            for (int x = -2; x <= 2; x++) {
                for (int z = -2; z <= 2; z++) {
                    for (int y = -2; y <= 2; y++) {
                        jumpY = Global::getClientInstance()->getLocalPlayer()->getStateVector()->Position.y;
                    }
                }
            }
        }
    ...
    void onEvent(ActorBaseTickEvent* event) override {
        ...
        case 1: // Flareon
            ...
            Global::getClientInstance()->getTimerClass()->setMainTimerSpeed(timer);
        ...
        if (mode == 0) {
            ...
            Global::getClientInstance()->getTimerClass()->setMainTimerSpeed(8);
            player->setSprinting(true);
