...
public:
    ...
    void onEvent(ActorBaseTickEvent* event) override {
        ...
        if (player == nullptr || !Global::getClientInstance()->getMinecraftGame()->getCanUseKeys()) return;
    ...
    void onEvent(ImGuiRenderEvent* event) override {
        ...
        for (auto& i : particles) {
            //Vec2 textpos = Global::getClientInstance()->WorldToScreen(i.pos);

            Vector2<float> textpos;
            if (!Global::getClientInstance()->WorldToScreen(i.pos, textpos)) continue;    

            if (0 < i.duration) {
                float dist = (i.pos).distance(Global::getClientInstance()->getLevelRender()->getOrigin()) * 3;
