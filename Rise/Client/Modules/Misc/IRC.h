    ...
    void onEvent(PacketEvent* event) {
        ...
        if (strcmp(event->Packet->getTypeName().getText(), "TextPacket") == 0) {
            ...
            if (Message->at(0) == '#') {
                ...
                ChatUtils::sendNormalMessage(Utils::combine(Utils::combine(BOLD, GRAY, "[IRC] ", RESET, DARK_GRAY, "[", GRAY, "shadow", DARK_GRAY, "] [", Global::Username, "] ", RESET, IRCMsg.c_str())));
                
                TestUtil::sendIRCMessage(IRCMsg, Global::Username.c_str(), "Shadow");
