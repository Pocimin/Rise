...
public:
	...
	int32_t getEnchantValue(EnchantType enchantType) {
		...
		static auto getEnchantsLevel = reinterpret_cast<getEnchantsLevel_t>(Utils::findSig(xorstr_("48 89 5c 24 ?? 48 89 74 24 ?? 57 48 83 ec ?? 48 8b f2 0f b6 d9")));
		return getEnchantsLevel(enchantType, this);
	}

	std::string getHoverName() {
		std::string name;
		static uintptr_t func = Utils::ResolveRef((uintptr_t)Utils::findSig("E8 ? ? ? ? 48 8D ? ? ? ? ? 48 8B ? E8 ? ? ? ? 48 8B ? ? ? 48 83 FA ? 0F 82"));
