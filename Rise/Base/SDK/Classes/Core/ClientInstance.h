...
public:
	...
	Player* getLocalPlayer() {
		return (Player*)Utils::CallVFunc<LocalPlayerIndex, Player*>(this); // 1.20.80
	}

	BlockSource* getBlockSource() {
		return Utils::CallVFunc<28, BlockSource*>(this); // 1.20.80
	}

	void setDisableInput(bool disable) { // grabMouse - 1
		return Utils::CallVFunc<ReleaseMouseIndex, void>(this, disable); // 1.20.80
	}

	void releaseMouse() { // grabMouse - 1
		return Utils::CallVFunc<ReleaseMouseIndex, void>(this); // 1.20.80
	}

	void grabMouse() {
		return Utils::CallVFunc<GrabMouseIndex, void>(this); // 1.20.80
	}

	Vector2<float> getFov() { // Check for fov in clientinstance
		Vector2<float> fov;
		fov.x = *(float*)(reinterpret_cast<uintptr_t>(this) + 0x6F0); // 1.20.80
		fov.y = *(float*)(reinterpret_cast<uintptr_t>(this) + 0x704); // 1.20.80
		return fov;
	}

	GLMatrix* getGLMatrix() {
		return (GLMatrix*)((uintptr_t)(this) + 0x330); // // 1.20.80
	}
...
public:
	BUILD_ACCESS(class LoopbackPacketSender*, LoopbackPacketSender, 0xF0); // 1.20.80
	BUILD_ACCESS(class MinecraftGame*, MinecraftGame, 0xC8); // 1.20.80
	BUILD_ACCESS(class TimerClass*, TimerClass, 0xD0); // 1.20.80
	BUILD_ACCESS(class GuiData*, GuiData, 0x558); // 1.20.80
	BUILD_ACCESS(class LevelRender*, LevelRender, 0xE0); // 1.20.80
};