...
public:
	...
	PlayerInventory* getSupplies() { 
		...
		return *reinterpret_cast<PlayerInventory**>(address + 0x760);
	}
	...
	std::string* getNametag() {
		...
		if (signature == NULL) {
			signature = Utils::findSig(xorstr_("48 83 EC ? 48 8B 81 ? ? ? ? 48 85 C0 74 3B 48 8B 08 BA ? ? ? ? 48 8B 40 ? 48 2B C1 48 C1 F8 ? 66 3B D0 73 17"));
		}
	...
	void setNametag(std::string* name) {
		...
		if (sig == NULL) {
			sig = Utils::findSig("48 89 5C 24 ? 57 48 83 EC ? 48 8B D9 48 8B FA 48 8B 89 ? ? ? ? 48 85 C9 0F 84");
		}
	...
	void setInvisible(bool invisible) {
		static void* SetInvisableFunc = Utils::findSig("48 89 74 24 ? 57 48 83 EC 50 48 8B 01 0F B6 F2 48 8B F9 48 8B 80 ? ? ? ? FF 15 ? ? ? ? 40 3A C6");
	...
	ItemStack* getArmor(int slot) {
		...
		if (sig == NULL) {
			sig = Utils::findSig("40 53 48 83 EC 20 48 83 C1 08 8B DA E8 ? ? ? ? 4C 8B C0 8B D3 48 8B 08 48 8B 41 28 49 8B C8 48 83 C4 20 5B 48 FF 25");
		}

		auto getArmor = reinterpret_cast<ItemStack*(__thiscall*)(Actor*, int)>(sig); // getArmor signature address
		return getArmor(this, slot); // Return FunctionCall as Actor
	}
	...
	void teleportTo(Vector3<float> position) {
		getStateVector()->Position = position;
	}
	...
	float getBodyRotation() {
		auto component = getComponent<MobBodyRotationComponent>();

		if (component != nullptr) {
			//FileUtils::debugOutput("[Rise] entt: MobBodyRotationComponent is not NULL");
			return component->bodyRot;
		}
		else {
			//FileUtils::debugOutput("[Rise] entt: MobBodyRotationComponent is NULL");
			return 0;
		}
    }
	...
	void setXHeadRotation(float rotation) {
		getComponent<ActorHeadRotationComponent>()->rotation.x = rotation;
	}

	void setYHeadRotation(float rotation) {
		getComponent<ActorHeadRotationComponent>()->rotation.y = rotation;
	}
	...
	int getHurtTime() {
		if (getComponent<MobHurtTimeComponent>() == nullptr) { return 0; }
	...
	{
		uintptr_t address = reinterpret_cast<uintptr_t>(this);
		return reinterpret_cast<EntityContext*>((uintptr_t)this + 0x8); // 1.21.0
	}
public:
	// Level Actor::getLevel(void)
	BUILD_ACCESS(class Level*, Level, 0x250); // 1.20.80
	// StateVectorComponent Actor::getPosition(void)
	BUILD_ACCESS(StateVectorComponent*, StateVector, 0x290); // 1.21.0
