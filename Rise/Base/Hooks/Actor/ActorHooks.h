...
void ActorRenderDispatcherDetour(void* _this, void* a2, Actor* actor, Vector3<float> cameraPos, Vector3<float> actorPosition, Vector2<float>* a6, bool renderCurrentPos) {
	if (actor == Global::getClientInstance()->getLocalPlayer()) {
		if (getModuleByName("desync")->isEnabled() || getModuleByName("fly")->isEnabled()) {
			...
			Utils::CallFunc<void*, void*, void*, Actor*, Vector3<float>, Vector3<float>, Vector2<float>*, bool>( // CallFunc to call the original.
...
bool IntersectsDetour(Actor* _this, Vector3<float> pos1, Vector3<float> pos2) {
	...
	// Inside our functiion we're calling the original code that was there/the original function we hooked so the games behavior doesn't change.
	return Utils::CallFunc<bool, Actor*, Vector3<float>, Vector3<float>>( // CallFunc to call the original.
...
bool isOnFireDetour(Actor* _this)
{
	...
	if (getModuleByName("nofire")->isEnabled()) {
		if (_this == Global::getClientInstance()->getLocalPlayer()) {
			return false;
		}
	}

	// Inside our functiion we're calling the original code that was there/the original function we hooked so the games behavior doesn't change.
	Utils::CallFunc<bool, Actor*>( // CallFunc to call the original.
		onIsOnFire, _this
	);
}
...
bool canBeAffected(Actor* _this, int duh) {
	...
	return Utils::CallFunc<bool, Actor*, int>(
...
float DestroySpeedDetour(Player* _this, Block& a1)
{
	...
	// Inside our functiion we're calling the original code that was there/the original function we hooked so the games behavior doesn't change.
	Utils::CallFunc<float, Player*, Block&>( // CallFunc to call the original.
...
public:
	...
	bool Initialize() override // Now override the Initialize function so we can actually hook our stuff
	{
		...
		void* ActorRenderDispatcherAddr = Utils::findSig(xorstr_("48 89 ? ? ? 55 56 57 41 ? 41 ? 41 ? 41 ? 48 8D ? ? ? 48 81 EC ? ? ? ? 48 8B ? ? ? ? ? 48 33 ? 48 89 ? ? 4D 8B ? 49 8B ? 48 8B ? 4C 8B ? 41 8B"));
		
		// Actor::intersects
		void* intersectsAddr = Utils::findSig(xorstr_("48 83 EC ? 48 8B 81 ? ? ? ? 48 85 C0 74 50"));

		// Actor::isImmobile
		void* isImmobileAddr = Utils::findSig(xorstr_("40 53 48 83 EC 20 48 8D 59 08"));
		
		// Player::getDestroySpeed
		void* destroySpeedAddr = Utils::findSig(xorstr_("48 89 5C ? ? 57 48 83 EC ? 48 8B FA ? ? 74 24 ? 48 8B 91"));
		...
		if (!Utils::HookFunction(ActorRenderDispatcherAddr, (void*)&ActorRenderDispatcherDetour, &onActorRenderDispatcher, xorstr_("ActorRenderDispatcher"))) { return false; }
		...
		if (!Utils::HookFunction(intersectsAddr, (void*)&IntersectsDetour, &onIntersects, xorstr_("Intersects"))) { return false; }

		// Attempt to hook the function at the address stored in isImmobileAddr by replacing it with the address of isImmobileDetour and store the original function's address in onIsImmobile
		if (!Utils::HookFunction((void*)VTable[43], (void*)&isImmobileDetour, &onIsImmobile, xorstr_("IsImmobile"))) { return false; }
		...
		if (!Utils::HookFunction((void*)VTable[51], (void*)&isOnFireDetour, &onIsOnFire, xorstr_("isOnFire"))) { return false; }

		if (!Utils::HookFunction((void*)VTable[111], (void*)&canBeAffected, &onCanBeEffected, xorstr_("canBeEffected"))) { return false; }
		...
		if (!Utils::HookFunction(destroySpeedAddr, (void*)&DestroySpeedDetour, &onDestroySpeed, xorstr_("DestroySpeed"))) { return false; }
