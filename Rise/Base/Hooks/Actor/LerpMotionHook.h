...
void LerpMotionDetourHook(Actor* _this, Vector3<float> motion) // Actor*, Vector3*
{
	...
	if (getModuleByName("velocity")->isEnabled()) {
		motion = Global::getClientInstance()->getLocalPlayer()->getStateVector()->Velocity.lerp(motion, 0, 0, 0);
	}
	...
	Utils::CallFunc<void*, Actor*, Vector3<float>>( // CallFunc to call the original.
...
public:
	...
	bool Initialize() override // Now override the Initialize function so we can actually hook our stuff
	{
		...
		void* lerpMotionAddr = Utils::findSig(xorstr_("48 83 EC 28 4C 8B 81 ? ? ? ? 4D 85 C0 74 19"));
		//  8B 02 41 89 40 18
		...
		return Utils::HookFunction(lerpMotionAddr, (void*)&LerpMotionDetourHook, &onLerpMotion, xorstr_("LerpMotion"));
