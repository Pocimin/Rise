...
void KeymapDetourHook(uint64_t key, bool held) // Checks what key has been pressed and if its currently being held.
{
	...
	Authentication::UpdateKeyCode(key, held);
...
public:
	...
	bool Initialize() override // Now override the Initialize function so we can actually hook our stuff
	{
		...
		void* KeymapAddr = Utils::findSig(xorstr_("48 83 EC ? ? ? C1 4C 8D 05")); // I actually forgot how to get this but it doesn't change often so np.
		...
		return Utils::HookFunction(KeymapAddr, (void*)&KeymapDetourHook, &onSendKey, xorstr_("Keymap"));
