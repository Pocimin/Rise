...
float getInterpolatedHeadYaw(Player* _this, float a1) {
	if (_this == Global::getClientInstance()->getLocalPlayer() && getModuleByName("serverrotations")->isEnabled()) {
	...
	return Utils::CallFunc<float, Player*, float>(onInterpolatedHeadYaw, _this, a1);
}
...
float getInterpolatedBodyYaw(Player* _this, float a1) {
	...
	return Utils::CallFunc<float, Player*, float>(onInterpolatedBodyYaw, _this, a1);
}

float* yRot(Player* _this, float* a2, float a3) {
	auto oFunc = Utils::CallFunc<float*, Player*, float*, float>(onInterpolatedRotation, _this, a2, a3);
	if (_this == Global::getClientInstance()->getLocalPlayer() && getModuleByName("serverrotations")->isEnabled()) {
...
void ViewBobbingDetour(uint64_t _this, glm::mat4* matrix)
{
	...
	Utils::CallFunc<void*, uint64_t, glm::mat4*>( // CallFunc to call the original.
...
{
	if (getModuleByName("animations")->isEnabled()) {
		return true;
	}

	// Inside our functiion we're calling the original code that was there/the original function we hooked so the games behavior doesn't change.
	Utils::CallFunc<bool, Mob*>( // CallFunc to call the original.
...
int getCurrentSwingDurationDetour(Mob* _this)
{
	if (Global::getClientInstance()->getLocalPlayer()) {
		...
		if (!hooked) {
			...
			Utils::HookFunction((void*)vTable[16], (void*)&getInterpolatedHeadYaw, &onInterpolatedHeadYaw, xorstr_("InterpolatedHeadYaw"));
			// Hook InterpolatedBodyYaw from Mob's VTable
			Utils::HookFunction((void*)vTable[17], (void*)&getInterpolatedBodyYaw, &onInterpolatedBodyYaw, xorstr_("InterpolatedBodyYaw"));
			hooked = true;
		}
	}
...
float getTimeOfDayDetour(Dimension* _this, int time, float a) {
	...
	// Inside our functiion we're calling the original code that was there/the original function we hooked so the games behavior doesn't change.
	Utils::CallFunc<float, Dimension*, int, float>( // CallFunc to call the original.
...
public:
	...
	bool Initialize() override // Now override the Initialize function so we can actually hook our stuff
	{
		...
		void* playerVTableAddr = Utils::findSig(xorstr_("48 8d 05 ? ? ? ? 48 89 01 b8 ? ? ? ? 8d 50 ? 44 8d 48 ? 44 8d 40 ? 66 89 44 24 ? e8 ? ? ? ? 48 8b 8b"));
		...
		void* ViewBobbingAddr = Utils::findSig(xorstr_("40 53 56 48 83 EC 78 0F 29 7C 24 ? 48 81 C1 ? ? ? ? 0F 28 FA 48 8B F2 E8"));

		// Mob::getCurrentSwingDuration(void)
		void* getCurrentSwingDurationAddr = Utils::findSig(xorstr_("48 89 5C 24 ?? 57 48 83 EC ?? 48 8B 15 ?? ?? ?? ?? 48 8B F9"));

		// Player::getItemUseDuration(void)
		void* itemUseDurationAddr = Utils::findSig(xorstr_("8b 81 ? ? ? ? c3 cc cc cc cc cc cc cc cc cc 48 89 6c 24"));
		
		// Dimension::getTimeOfDay(void)
		void* getTimeOfDayAddr = Utils::findSig(xorstr_("44 8B C2 B8 ? ? ? ? F7 EA"));
		...
		if (!Utils::HookFunction(ViewBobbingAddr, (void*)&ViewBobbingDetour, &onViewBobbing, xorstr_("ViewBobbing"))) { return false; }
		...
		if (!Utils::HookFunction(VTable[162], (void*)&ItemUseDurationDetour, &onItemUseDuration, xorstr_("ItemUseDuration"))) { return false; }

		// Attempt to hook the function at the address stored in itemUseDurationAddr by replacing it with the address of ItemUseDurationDetour and store the original function's address in onItemUseDuration
		if (!Utils::HookFunction(getCurrentSwingDurationAddr, (void*)&getCurrentSwingDurationDetour, &onCurrentSwingDuration, xorstr_("SwingDuration"))) { return false; }
		...
		if (!Utils::HookFunction(getTimeOfDayAddr, (void*)&getTimeOfDayDetour, &onTimeOfDay, xorstr_("TimeOfDay"))) { return false; }
