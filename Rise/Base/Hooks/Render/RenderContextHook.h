...
void HoverRenderDetour(HoverTextRenderer* a1, MinecraftUIRenderContext* renderContext, ClientInstance* client, UIControl* owner, int pass, Vector4<float>* renderAABB) {
    ...
    Utils::CallFunc<void, HoverTextRenderer*, MinecraftUIRenderContext*, ClientInstance*, UIControl*, int, Vector4<float>*>(
...
void onDrawTextDetour(MinecraftUIRenderContext* _this, FontRepos* font, Vector4<float> pos, std::string* str, UIColor const& colour, float alpha, float alinM, TextMeasureData const& textdata, CaretMeasureData const& caretdata) {
    std::string damn1 = "Error:";
    std::string damn = "Malformed Input.";

    static std::string Nametag = "Not found";

    if (Global::getClientInstance()->getLocalPlayer() != nullptr)
        Nametag = Global::getClientInstance()->getLocalPlayer()->getNametag()->c_str();

    std::string admin = Utils::combine(GRAY, "   You've been kicked by an", RED, " admin.");

    float RenderX = Global::getClientInstance()->getGuiData()->mcResolution.x / 2; // The X Position (center)
    float RenderY = (Global::getClientInstance()->getGuiData()->mcResolution.y / 2) - 40; // The Y Position (center)

    std::string KickCode = Utils::combine(DARK_RED, "Flareon", GRAY, " kicked you: \n", DARK_GRAY, str->c_str()).c_str();
    float KickCodeWidth = _this->getLineLength(font, &KickCode, 1.1, false);

    Vector2<float> KickCodePos(0, 0);

    if (str->find("Error:") != std::string::npos) {
        //float strSize = (str->size() - 20) / 2;
        //DiscordUtil::sendAntiCheatMessage(Utils::combine("> ", Nametag, " (", Global::Username.c_str(), ")", " has been kicked by flareon"), "Kick Decryptor");
        //DiscordUtil::sendAntiCheatMessage(Utils::combine("> Kick Error for ", Nametag, " (", Global::Username.c_str(), ") : ", str->c_str()), "Kick Decryptor");
        //str = &KickCode;
    }

    if (str->find("You've been kicked from the server.") != std::string::npos) {
        //DiscordUtil::sendAntiCheatMessage(Utils::combine("> ", Nametag, " (", Global::Username.c_str(), ")", " has been kicked from the server by an admin"), "Kick Decryptor");
        //DiscordUtil::sendAntiCheatMessage(Utils::combine("> Kick Error for ", Nametag, " (", Global::Username.c_str(), ") : ", str->c_str()), "Kick Decryptor");
        //str = &admin;
    }
    ...
    Utils::CallFunc<void, MinecraftUIRenderContext*, FontRepos*, Vector4<float>, std::string*, UIColor const&, float, float, TextMeasureData const&, CaretMeasureData const&>(
...
void renderDetourHook(ScreenView* __this, MinecraftUIRenderContext* ctx) { // ScreenContext, MinecraftUIRenderContext
    ...
    if (UILayer::Is(__this, UILayer::MainMenu_Screen))
    {
        //return;
        ...
        Vector4<float> rectPos(0, 0, 135, 170);

        // Call the fillRectangle function on ctx, passing in rectPos and color as arguments
        //ctx->fillRectangle(rectPos, color, color.a);
        //RenderUtils::renderImage("lmao", Vector2<float>(0, 0), Vector2<float>(300, 300), Type::Internal);
    }

    Global::LayerName = __this->tree->root->GetName();
    LayerEvent layerEvent{ __this }; // LayerEvent
    layerEvent.cancelled = nullptr;
    CallBackEvent(&layerEvent); // Call Layer event for modules to be writen on this hook.
    ...
    if (!tryHookDrawImage) {
        ...
        tryHookDrawImage = Utils::HookFunction((void*)vtable[7], (void*)&onDrawImageDetour, &onDrawImage, xorstr_("DrawImage"));
    }

    if (!tryHookDrawNineSlice) {
        // Hook drawImage from MinecraftUIRenderContext's VTable
        tryHookDrawNineSlice = Utils::HookFunction((void*)vtable[8], (void*)&onDrawNineSliceDetour, &onDrawNineSlice, xorstr_("DrawNineSlice"));
    }
...
public:
    bool Initialize() override
    {
        ...
        void* renderAddr = Utils::findSig(xorstr_("48 8B C4 48 89 58 18 55 56 57 41 54 41 55 41 56 41 57 48 8D A8 ? ? ? ? 48 81 EC ? ? ? ? 0F 29 70 B8 0F 29 78 A8 48 8B 05 ? ? ? ? 48 33 C4 48 89 85 ? ? ? ? 4C 8B FA"));
        ...
        void* hoverRenderAddr = Utils::findSig(xorstr_("48 8B C4 48 89 58 ? 48 89 70 ? 48 89 78 ? 4C 89 70 ? 55 48 8D 68 ? 48 81 EC ? ? ? ? 0F 29 70 ? 0F 29 78 ? 44 0F 29 40 ? 49 8B D9"));
        ...
        if (!Utils::HookFunction(renderAddr, (void*)&renderDetourHook, &onRender, xorstr_("RenderContext"))) { return false; };
        
        // Attempt to hook the function at the address stored in renderAddr by replacing it with the address of renderDetour and store the original function's address in onRender
        //if (!Utils::HookFunction(hoverRenderAddr, (void*)&HoverRenderDetour, &onHoverRenderHook, xorstr_("HoverRender"))) { return false; };
