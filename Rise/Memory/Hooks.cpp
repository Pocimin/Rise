...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx)
{
...

	{
		...
		Vector2 windowSize = g_Data.getClientInstance()->getGuiData()->windowSize;
		Vector2 windowSizeReal = g_Data.getClientInstance()->getGuiData()->windowSizeReal;

		Vector2 mousePos = *g_Data.getClientInstance()->getMousePos();
		...
		if (moduleMgr->isInitialized() && shouldRenderArrayList) {
			...
			for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
				...
				DrawUtils::drawText(textPos, &textStr, new MinecraftColor(0.3f, 0.3f, 0.3f, 1.f), textSize);
				if (!GameData::canUseMoveKeys() && rectPos.contains(&mousePos)) {

					if (leftMouseDown) {
						DrawUtils::fillRectangle(rectPos, MinecraftColor(0.4f, 0.9f, 0.4f, 0.1f), it->enabled ? 0.6f : 0.6f);
						if (executeClick)
							it->backingModule->toggle();
					}
					else
						DrawUtils::fillRectangle(rectPos, MinecraftColor(0.3f, 0.7f, 0.3f, 0.1f), it->enabled ? 0.4f : 0.15f);
				}
		...
		if (moduleMgr->isInitialized() && shouldRenderCoords && g_Data.getLocalPlayer() != nullptr) {
			Vector3* pos = g_Data.getLocalPlayer()->getPos();
			std::string coords = "" + std::to_string((int)pos->x) + ", " + std::to_string((int)pos->y) + ", " + std::to_string((int)pos->z);
			DrawUtils::drawText(Vector2(5.f, shouldRenderTabGui ? windowSize.y - 12.f : 10.f), &coords, nullptr, 1.f);
		}
	}
...
float Hooks::Dimension_getSunIntensity(__int64 a1, float a2, Vector3* a3, float a4)
{
	static auto oGetSunIntensity = g_Hooks.Dimension_getSunIntensityHook->GetFastcall<float, __int64, float, Vector3*, float>();
...
void Hooks::Actor_lerpMotion(C_Entity* _this, Vector3 motVec)
{
	static auto oLerp = g_Hooks.Actor_lerpMotionHook->GetFastcall<void, C_Entity*, Vector3>();
...
void Hooks::GameMode_startDestroyBlock(C_GameMode* _this, Vector3i* a2, uint8_t face, void* a4, void* a5)
{
	static auto oFunc = g_Hooks.GameMode_startDestroyBlockHook->GetFastcall<void, C_GameMode*, Vector3i*, uint8_t, void*, void*>();
